...
{{ Auth::user()->status == 'active' ? 'background:#28a745;color:white;' : 'background:red;color:white;' }}
    ...
    ">
                    ...
                    @else
                        <i class="fas fa-user {{ $isInactive ? 'text-danger' : 'text-success' }}" style="font-size:18px;">
                        </i>
                    @endif
            ...
            <div class="row mb-4 flex-nowrap overflow-auto pb-1">
                @php
                    $stats = [
                        ...
                        [
                            'label' => 'Lost Income',
                            'value' => '₹' . number_format($dashStats['lost_income'], 2),
                            'sub' => 'Exceeded plan limit',
                            'icon' => 'fa-ban',
                            'color' => '#dc3545',
                        ],
                    ];
                ...
                @foreach ($stats as $stat)
                    <div style="min-width:180px;flex:1;" class="px-2 mb-1">
                        <div class="stat-card d-flex align-items-center gap-3"
                            ...
                            <div>
                                ...
                                <div style="font-size:11px;color:#aaa;margin-top:2px;">{{ $stat['sub'] }}</div>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        ">
                    <div>
                        <div style="font-size:15px;font-weight:700;color:#b02a37;margin-bottom:6px;">
                            ⚠️ Earnings Limit Reached
                        </div>
                        <div style="font-size:13px;color:#555;line-height:1.7;">
                            Plan: <strong>₹{{ number_format($plan->price, 0) }}</strong>
                            &rarr; Max Limit: <strong>₹{{ number_format($totalCap, 0) }}</strong><br>
                            Current Earnings: <strong>₹{{ number_format($totalEarned, 2) }}</strong><br>
                            @if ($dailyCapHit)
                                Daily Limit: <strong>₹{{ number_format($dailyCap, 0) }}</strong>
                                &nbsp;|&nbsp; Today Earned: <strong>₹{{ number_format($todayEarned, 2) }}</strong>
                            @endif
                        </div>
                        @if ($dailyCapHit && !$totalCapHit)
                            <div style="margin-top:6px;color:#dc3545;font-size:12px;font-weight:600;">
                                ₹{{ number_format(max(0, $lostIncome), 2) }} will be LOST after 11:59 PM
                            </div>
                        @endif
                    </div>
                    <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap;">
                        <div style="font-size:17px;font-weight:800;color:#333;">
                            Reset in: <span id="capCountdown" style="color:#28a745;">--:--:--</span>
                        </div>
                        <div style="display:flex;gap:8px;">
                            ...
                            <button type="button" class="btn btn-sm btn-round-full"
                                ...
                                data-toggle="modal" data-target="#upgradeModal">View Plans</button>
                        </div>
                    </div>
                </div>
            @endif
        ...
        @else
            #dc3545 @endauth;">
                    ...
                    {{ Auth::user()->status == 'inactive' ? 'text-danger' : 'text-success' }}">
                        ...
                        ">
                                    <div
                                        style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:#7b5900;">
                                        <i class="fas fa-lock" style="color:#f59e0b;font-size:14px;"></i>
                                        Locked Earnings: <span
                                            style="color:#b45309;">₹{{ number_format($lockedEarning, 2) }}</span>
                                    </div>
                                    <div style="font-size:11px;color:#92400e;margin-top:4px;">
                                        @if ($nextPlan)
                                            Upgrade to <strong>{{ $nextPlan->name }}</strong> to unlock earnings.
                                            <a href="#" data-toggle="modal" data-target="#upgradeModal"
                                                style="color:#d97706;font-weight:700;text-decoration:underline;">Upgrade
                                                Now</a>
                                        @else
                                            You've reached the maximum plan. Contact support.
                                        @endif
                                    </div>
                                </div>
                            @endif
                ...
                <div class="modal-body" style="padding:24px;">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="invite-code-box">
                            <input type="text" id="referralCode" value="{{ auth()->user()->reference_code }}"
                                readonly>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="invite-code-box">
                            <input type="text" id="referralLink"
                                value="{{ url('/register?ref=' . auth()->user()->reference_code) }}" readonly>
                            <button class="btn btn-main btn-sm px-3"
                    ...
                    <div class="row" style="gap:0;">
                        <div class="col-6 mb-2">
                            <a id="shareWhatsapp"
                                href="https://wa.me/?text={{ urlencode('Join me using my referral link: ' . url('/register?ref=' . auth()->user()->reference_code)) }}"
                        ...
                        <div class="col-6 mb-2">
                            <a id="shareGmail"
                                href="https://mail.google.com/mail/?view=cm&fs=1&su=Join Me&body={{ urlencode('Join me using my referral link: ' . url('/register?ref=' . auth()->user()->reference_code)) }}"
                                target="_blank" class="share-btn gmail">
                                <i class="fas fa-envelope fa-lg"></i> Gmail
                            </a>
                        </div>
                        <div class="col-6 mb-2">
                            <a id="shareFacebook"
                                href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url('/register?ref=' . auth()->user()->reference_code)) }}"
                        ...
                        <div class="col-6 mb-2">
                            <a id="shareTwitter"
                                href="https://twitter.com/intent/tweet?text=Join me using my referral link: {{ urlencode(url('/register?ref=' . auth()->user()->reference_code)) }}"
                        ...
                        <div class="col-6 mb-2">
                            <a id="shareTelegram"
                                href="https://t.me/share/url?url={{ urlencode(url('/register?ref=' . auth()->user()->reference_code)) }}&text=Join Me"
...
@push('scripts')
    ...
    <script>
        ...
        (function() {
            var el = document.getElementById('capCountdown');
            if (!el) return;
    ...
    <script>
        (function() {
            const COMMISSION_URL = '{{ url('member/commissions') }}';
            let currentPage = 1;
